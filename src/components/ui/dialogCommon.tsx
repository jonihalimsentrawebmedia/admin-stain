...
>(({ className, position, children, ...props }, ref) => {
  ...
  switch (position) {
    ...
    case 'bottom':
      ...
      break
    case 'middle':
      contentClassName += ' top-[50%] left-[50%] transform translate-x-[-50%] translate-y-[-50%]'
      break
    default:
      contentClassName += ' top-[50%] left-[50%] transform translate-x-[-50%] translate-y-[-50%]'
  }
  ...
  return (
    <DialogPortal>
      <DialogOverlay className="z-99" position={position} />
      <DialogPrimitive.Content ref={ref} className={cn(contentClassName, className)} {...props}>
        {children}
