...
const CalendarAcademicBackgroundView = () => {
  ...
  async function handleUpdateStatus(data: ImageCalendarAcademic, index: number) {
    try {
      const res = await AxiosClient.patch(
        `/website-utama/tahun-akademik-background/${data.id_background}/status`,
        {
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-4 ">
        {image.map((item, index) => (
          <ImageUploader
            keyUrl="bg-calendar-academic"
            urlBg="/website-utama/tahun-akademik-background"
            urlBgDelete={`/website-utama/tahun-akademik-background/${item.id_background}`}
            handleChecked={() => {
...
export default CalendarAcademicBackgroundView
