...
export const ButtonDeleteGuide = (props: Props) => {
  const { data } = props
  ...
  const HandleSave = async () => {
    setLoading(true)
    await AxiosClient.delete(`/panduan/${valueGuide}/${data?.id_dokumentasi}`)
      .then((res) => {
        if (res.data.status) {
          setOpen(false)
          setLoading(false)
          ...
          toast.success(res.data.message || 'Success Delete Panduan')
        }
      })
      .catch((err) => {
        setLoading(false)
