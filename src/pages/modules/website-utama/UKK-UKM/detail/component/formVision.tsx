...
import { UseGetVisionMissionUkkUkm } from '../hooks/index'

const VisionMissionForm = () => {
  const { id } = useParams()
  const { visionMission } = UseGetVisionMissionUkkUkm((id as string) ?? '')
  ...
  const handleSave = async (e: IVisiMisiTypeForm) => {
    setLoading(true)
    await AxiosClient.post(`/website-utama/ukk-ukm-visi-misi/${id}`, {
      ...e,
    })
      .then((res) => {
        if (res.data.status) {
          queryClient.invalidateQueries({
            queryKey: ['vision-ukk-ukm'],
          })
          setLoading(false)
          setIsEdit(!isEdit)
