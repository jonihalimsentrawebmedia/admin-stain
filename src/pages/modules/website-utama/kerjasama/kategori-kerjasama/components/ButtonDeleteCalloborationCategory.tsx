...
const ButtonDeleteCalloborationCategory = ({ data }: Props) => {
  const form = useForm()
  ...
  const handleDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/website-utama/kategori-kerjasama/${data?.id_kategori_kerjasama}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
...
export default ButtonDeleteCalloborationCategory
