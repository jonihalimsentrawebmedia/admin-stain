...
const ButtonDeleteFieldOfCooperation = ({ data }: Props) => {
  const form = useForm()
  ...
  const handleDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/website-utama/bidang-kerjasama/${data?.id_bidang_kerjasama}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
...
export default ButtonDeleteFieldOfCooperation
