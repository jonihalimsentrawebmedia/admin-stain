...
import type { ICategoryFAQ } from '../data/type.ts'
...
export const ButtonEditCategoryFAQ = (props: Props) => {
  const { data } = props
  ...
  const HandleSave = async (value: any) => {
    setLoading(!loading)
    await AxiosClient.put(`/website-utama/kategori-faq/${data?.id_kategori_faq}`, value)
      .then((res) => {
        if (res.data.status) {
          setOpen(false)
          setLoading(false)
