...
import type { NonUktEntranceList } from '../data/types.ts'

interface props {
  data: NonUktEntranceList
}

export const ButtonDeleteEntranceNonUkt = (props: props) => {
  const { data } = props
  ...
  const handleSave = async () => {
    setLoading(true)
    await AxiosClient.delete(`/website-utama/jalur-masuk-non-ukt/${data?.id_non_ukt_jalur_masuk}`)
      .then((res) => {
        if (res.data.status) {
    ...
    <>
      ...
      >
        <div className={'grid grid-cols-[12rem_1fr] gap-5'}>
          ...
          <p>{data?.nama_prodi}</p>
          <p className="text-gray-500">Nama Fakultas</p>
          <p>{data?.nama_fakultas}</p>
          <p className="text-gray-500">Nama Jenjang</p>
          <p>{data?.nama_jenjang}</p>
