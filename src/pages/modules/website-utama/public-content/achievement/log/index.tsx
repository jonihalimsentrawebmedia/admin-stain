...
import { UseGetAchievementDetail, UseGetLogAchievement } from '../hooks/index'

export const LogActivityAchievement = () => {
  const { id } = useParams()
  const columns = ColumnsLog()
  const { logData } = UseGetLogAchievement(id ?? '')
  ...
  return (
    <>
      ...
      <div className={'flex flex-col lg:flex-row items-start gap-x-8 mt-5'}>
        <img src={detail?.gambar} className={'w-full lg:w-[320px] h-[240px] object-contain'} alt="imgae" />
        <div className={'flex flex-col gap-y-2 text-sm'}>
          <p className="text-gray-500">Judul Prestasi</p>
          <p>{detail?.judul}</p>
