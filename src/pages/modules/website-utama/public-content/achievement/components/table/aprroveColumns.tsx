...
import { ButtonPublishAchievement } from '@/pages/modules/website-utama/public-content/achievement/components/buttonPublish.tsx'

export const ApprovedColumnsAchievement = () => {
  const [searchParams] = useSearchParams()
  const page = Number(searchParams.get('page') ?? 1)
  const limit = Number(searchParams.get('limit') ?? 10)

  const columns: ColumnDef<IAchievementDetail>[] = [
    {
      accessorKey: 'No',
      header: '#',
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: 'Aksi',
      cell: ({ row }) => {
        return <ButtonPublishAchievement {...row?.original} />
      },
    },
    {
      accessorKey: 'log',
      ...
      cell: ({ row }) => {
        return (
          <>
            <Link to={`log/${row?.original?.id_prestasi}`}>
              <Button
                size={'sm'}
                variant={'outline'}
