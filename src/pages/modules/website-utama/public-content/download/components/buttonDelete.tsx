...
import type { IDownload } from '../types/index'
...
export const ButtonDeleteFileDownload = (props: props) => {
  ...
  const HandlerDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/website-utama/downloads/${data?.id_download}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
          toast.success(res.data.message || 'Success menghapus data Download')
          queryClient.invalidateQueries({ queryKey: ['list-download'] })
        }
      })
      .catch((err) => {
        setLoading(false)
