...
import { FormLanguageDownload } from './component/form'

export const DownloadFileLanguage = () => {
  const { id } = useParams()
  ...
  const HandleSave = async (e: any) => {
    setLoading(true)
    await AxiosClient.post(`/website-utama/downloads-translate/${id}`, {
      language: language,
      nama_berkas: e?.nama_berkas,
    })
      .then((res) => {
        if (res.data.status) {
          setLoading(false)
  ...
  const tabsData = [
    {
      ...
      value: 'id',
      element: <FormLanguageDownload loading={loading} form={form} HandleSave={HandleSave} />,
    },
    {
      id: 2,
      name: 'English',
