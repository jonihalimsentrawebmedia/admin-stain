...
const BottomRejectEditorColumns = () => {
      ...
      cell: ({ row }) => {
        return (
          <div className={'flex flex-col gap-1.5 items-center'}>
            ...
            <p className={'text-xs'}>{format(row?.original?.ditolak_at as string, 'HH:mm:ss')}</p>
            <p className={'text-xs text-primary'}>{TimeAgo(row?.original?.ditolak_at as string)}</p>
          </div>
        )
      },
    },
...
export default BottomRejectEditorColumns
