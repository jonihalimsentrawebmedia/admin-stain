...
import { UseGetNotification } from '@/pages/modules/website-utama/component/layout/notification/hooks'
...
export const NotificationList = () => {
  ...
  const HandleReply = async (value: IMessageResolver) => {
    ...
    await AxiosClient.post(
      `/website-utama/pertanyaan/${data?.id_pertanyaan}/kirim-jawaban`,
      value
    )
      .then((res) => {
        if (res.data.status) {
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span
                    className={`w-2 h-2 mt-1 rounded-full ${
                      row?.status === 'UNREAD' ? 'bg-blue-500' : 'bg-gray-300'
                    }`}
                  />
                </div>
              </div>
