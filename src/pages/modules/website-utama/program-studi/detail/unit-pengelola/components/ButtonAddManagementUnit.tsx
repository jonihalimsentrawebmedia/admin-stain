...
import { ManagementUnitResolver, type IManagementUnitTypeForm } from '../../model/management-unit'
...
const ButtonAddManagementUnit = () => {
  ...
  const handleSave = async (e: IManagementUnitTypeForm) => {
    setLoading(true)
    await AxiosClient.post(`/website-utama/satuan-organisasi/${id}/unit-pengelola`, {
      ...e,
    })
      .then((res) => {
        if (res.data.status) {
