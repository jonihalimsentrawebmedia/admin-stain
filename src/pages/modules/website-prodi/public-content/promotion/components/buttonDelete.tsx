...
export const ButtonDeletePromotionProdi = (data: IPromotion) => {
  ...
  const HandlerDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/prodi/promosi/${data?.id_promosi}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
          toast.success(res.data.message || 'Success menghapus data Promosi')
  ...
  return (
    <>
      ...
      <DialogCustom
        ...
        description={'Apakah anda yakin untuk menghapus data Promosi ini?'}
      >
