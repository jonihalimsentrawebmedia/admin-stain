...
import { UseGetLogPromotionProdi, UseGetPromotionProdiDetail } from '../hooks/index'

export const LogActivityPromotionProdiPage = () => {
  const { id } = useParams()
  const { logData, loading, meta } = UseGetLogPromotionProdi(id ?? '')
  ...
  return (
    <>
      ...
      <div className={'flex flex-col lg:flex-row items-start gap-6 lg:gap-x-8 mt-5'}>
        <img src={detail?.gambar} className={'w-full lg:w-[320px] h-[240px] object-cover'} alt="imgae" />
      ...
      <TableCustom data={logData} loading={loading} meta={meta} columns={columns} />
    </>
  )
}
