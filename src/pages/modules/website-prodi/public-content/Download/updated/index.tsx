...
import { UseGetCategoryDownloadProdi, UseGetDownloadProdiDetail } from '../hooks/index'
import {
  DownloadResolver,
  type DownloadType,
} from '@/pages/modules/website-utama/public-content/download/types/resolver.tsx'

export const UpdatedDownloadProdiPage = () => {
  const { id } = useParams()
  const { downloadProdiDetail: detail } = UseGetDownloadProdiDetail(id ?? '')
  const [loading, setLoading] = useState(false)

  const { categoryDownload, loading: load1 } = UseGetCategoryDownloadProdi({ isGetAll: true })
  ...
  const HandleSubmit = async (e: DownloadType) => {
    setLoading(true)
    await AxiosClient.put(`/prodi/downloads/${detail?.id_download}`, e)
      .then((res) => {
        if (res?.data?.status) {
          setLoading(false)
          navigate('/modules/website-prodi/public-content/download')
