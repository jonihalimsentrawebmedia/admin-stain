...
const LecturerColumnsProfile = () => {
  ...
  async function checkedHp(idStaff: string) {
    await AxiosClient.patch(`/prodi/profil/dosen/${idStaff}/no-telepon`, {})
      .then((res) => {
        if (res.data.status) {
  ...
  async function checkedEmail(idStaff: string) {
    await AxiosClient.patch(`/prodi/profil/dosen/${idStaff}/email`, {})
      .then((res) => {
        if (res.data.status) {
          queryClient.invalidateQueries({
            queryKey: ['lecturer-profile'],
...
export default LecturerColumnsProfile
