...
import type { IThemeProdi } from '../data/types.ts'
...
const ButtonEditDescription = (props: Props) => {
  ...
  const HandleSave = async (e: any) => {
    setLoading(true)
    await AxiosClient.post(`/prodi/thema/${data?.thema}/keterangan`, e)
      .then((res) => {
        if (res.data.status) {
          setLoading(false)
          setOpen(false)
          toast.success(res.data.message || 'Success Pengajuan update data keterangan thema')
          queryClient.invalidateQueries({ queryKey: ['template-prodi'] })
        }
      })
      .catch((err) => {
        setLoading(false)
    ...
    <>
      <div className={'flex items-center gap-2'}>
        ...
        <button
          ...
          disabled={loading || !data?.is_tersedia}
          className={
