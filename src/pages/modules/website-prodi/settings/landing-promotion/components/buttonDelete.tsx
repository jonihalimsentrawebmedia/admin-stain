...
export const ButtonDeleteLandingPromotion = (props: props) => {
  ...
  const HandleSave = async () => {
    setLoading(true)
    await AxiosClient.delete(`/prodi/landing-page-promosi/${data?.id_prodi_landing_page_promosi}`)
      .then((res) => {
        if (res.data.status) {
          setOpen(false)
          setLoading(false)
  ...
  return (
    <>
      ...
      <DialogCustom
        open={open}
        setOpen={setOpen}
        title={<p className={'text-red-500'}>Hapus Gambar</p>}
        description={'Apakah anda yakin untuk menghapus data gambar ini?'}
