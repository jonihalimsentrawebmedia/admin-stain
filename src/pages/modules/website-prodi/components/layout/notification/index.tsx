...
import { UseGetNotificationProdi } from '../notification/hooks/index'

export const NotificationListProdi = () => {
  const { notification } = UseGetNotificationProdi()
  ...
  const HandleReply = async (value: IMessageResolver) => {
    setLoading(true)
    await AxiosClient.post(`/prodi/pertanyaan/${data?.id_pertanyaan}/kirim-jawaban`, value)
      .then((res) => {
        if (res.data.status) {
          setLoading(false)
          setOpen(false)
