...
import {
  UseGetAccreditationProdiById,
  UseGetAccreditationProdiLog,
} from '@/pages/modules/website-prodi/accreditation/hooks'
import { useParams } from 'react-router-dom'
import type { LogStatistic } from '@/pages/modules/website-utama/statistic/model'
import { capitalizeTextSimple } from '@/pages/modules/website-utama/acreditation/AcreditationViewModel.tsx'

export function isImageUrl(url: string) {
  return /\.(jpg|jpeg|png|gif|webp|bmp|svg)$/i.test(url)
}

const AccreditationProdiLog = () => {
  const { id } = useParams()
  const { accreditation, loading } = UseGetAccreditationProdiById(id ?? '')

  const { accreditationLog: log, loading: loadingLog, meta } = UseGetAccreditationProdiLog(id ?? '')

  const createdAt = formatDateTime(accreditation?.created_at ?? null)
  const updatedAt = formatDateTime(accreditation?.updated_at ?? null)
  const startAt = formatDateTime(accreditation?.mulai_berlaku ?? null)
  const endAt = formatDateTime(accreditation?.akhir_berlaku ?? null)
        ...
        <div>
          <div>
            ...
            <div className="text-green-600 font-medium text-3xl">
              {accreditation?.nama_satuan_organisasi_akreditas}
            </div>
            <div>
              <div className="text-[#999999] text-sm">Uraian</div>
              <div className="">{accreditation?.uraian}</div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <div className="text-[#999999] text-sm">Nilai Akreditasi*</div>
                <div className="">{capitalizeTextSimple(accreditation?.nilai_akreditas ?? '')}</div>
              </div>
              <div>
                <div className="text-[#999999] text-sm">Lembaga Penilai*</div>
                <div className="">{accreditation?.lembaga_penilaian}</div>
              </div>
              <div>
                <div className="text-[#999999] text-sm">No. Surat Keputusan*</div>
                <div className="">{accreditation?.no_surat_keputusan}</div>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="">
                  {accreditation?.nama_user_created}, {createdAt.date}, {createdAt.time}
                </div>
              </div>
              <div>
                <div className="text-[#999999] text-sm">Diperbaharui Oleh*</div>
                <div className="">
                  {accreditation?.nama_user_updated}, {updatedAt.date}, {updatedAt.time}
                </div>
              </div>
            </div>
          </div>
...
export default AccreditationProdiLog
