...
const ButtonDeleteAccreditationProdi = ({ data }: Props) => {
  const form = useForm()
  ...
  const handleDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/prodi/akreditas/${data?.id_akreditas}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
...
export default ButtonDeleteAccreditationProdi
