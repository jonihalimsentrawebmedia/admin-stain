...
export const UseGetTotalVisitorProdi = () => {
  ...
  const { data, isFetching, isLoading } = useQuery<ITotalVisitor>({
    ...
    queryFn: () =>
      AxiosClient.get('/prodi/dashboard/total-pengunjung').then((res) => res.data?.data),
  })
...
export const UseGetApprovedListProdi = (status: string) => {
  ...
  const { data, isLoading, isFetching } = useQuery<null | IContent[]>({
    queryKey: ['list-approved-prodi', ParamsSearch.toString()],
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get(`/prodi/dashboard/list-konten-pengajuan?${ParamsSearch}`).then(
        (res) => res.data.data
      ),
  })
...
export const UseGetTrentVisitorProdi = (mode: Mode) => {
  ...
  const { data, isLoading, isFetching } = useQuery({
    ...
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get(`/prodi/dashboard/statistik-pengunjung/${mode ?? 'harian'}`).then(
        (res) => res.data?.data
      ),
  })
