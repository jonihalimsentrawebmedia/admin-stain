...
import {
  UseGetGalleryVideoProdiById,
  UseGetGalleryVideoProdiLog,
} from '../hooks/index'

export const LogActivityVideoProdi = () => {
  const { id } = useParams()
  const columns = ColumnsLog()
  const { videoProdiLog: logData } = UseGetGalleryVideoProdiLog(id ?? '')
  const { detailGalleryVideo: detail } = UseGetGalleryVideoProdiById(id ?? '')
