...
import { UseGetLogTopSliderEditor, UseGetSliderDetailEditor } from '../hooks/index'
...
export const LogActivityPage = () => {
  ...
  const { logData } = UseGetLogTopSliderEditor(id ?? '')
    ...
    <>
      ...
      <div className={'flex items-start gap-x-8'}>
        ...
        <div className={'flex flex-col gap-y-2 text-sm'}>
          ...
          <p>
            {detailSlider?.nama_user_created ?? '-'},{' '}
            {detailSlider?.created_at
              ? format(detailSlider?.created_at, 'dd-MM-yyyy, HH:mm:ss')
              : '-'}
          </p>
          ...
          <p>
            {detailSlider?.nama_user_updated ?? '-'},{' '}
