...
import { UseGetArticleManagementEditorDetail } from '../hooks'
import { ResolverArticle, type IResolverArticleType } from '../data/resolver'
import { FormArticleLppm } from '@/pages/modules/LPPM/public-content/article/components/form'

export const UpdatedArticleManagementEditor = () => {
  const { id } = useParams()
  const { articleDetail: detail } = UseGetArticleManagementEditorDetail(id ?? '')
  ...
  const HandleSave = async (e: IResolverArticleType) => {
    ...
    await AxiosClient.put(`/editor/artikel/${detail?.id_artikel}`, e)
      .then((res) => {
        if (res.data.status) {
          setLoading(false)
          toast.success(res.data.message || 'Success Pengajuan tambah data promosi')
          navigate('/modules/editor/public-content/article')
        }
      })
      .catch((err) => {
