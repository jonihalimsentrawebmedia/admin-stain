...
export const ButtonDeletePoli = (props: Props) => {
  const { data } = props
  ...
  const HandleSave = async () => {
    setLoading(true)
    await AxiosClient.delete(`/simrs/referensi/poli/${data?.id_poli}`)
      .then((res) => {
        if (res.data.status) {
          toast.success(res.data.message || 'Berhasil menghapus data poli')
    ...
    <>
      ...
      >
        <div className={'grid gap-5 grid-cols-[12rem_1fr]'}>
          <p className="text-gray-500">Nama Poli</p>
          <p>{data.nama}</p>
          <p className="text-gray-500">Lokasi</p>
          <p>{data.lokasi}</p>
          <p className="text-gray-500">Status</p>
          <p>{data.is_status ? 'Aktif' : 'Tidak Aktif'}</p>
        </div>
