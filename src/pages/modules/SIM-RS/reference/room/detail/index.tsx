...
import { UseGetDetailRoom } from '../hooks/index.tsx'

const DetailRoom = () => {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { detail, loading } = UseGetDetailRoom(id ?? '')
          ...
          buttonGroup={[
            {
              ...
              label: 'Edit',
              onClick: () =>
                navigate(`/modules/sim-rs/reference/room/edit/${detail.id_ruangan}`),
            },
          ]}
        />
        ...
        <div className="bg-white rounded-lg border p-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <p className="text-base font-medium">{detail.nomor}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Jenis Ruangan</p>
              <p className="text-base font-medium">{detail.nama_jenis_ruangan}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Jumlah Kasur</p>
