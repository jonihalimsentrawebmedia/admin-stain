...
import {
  ...
  UserProfileResolver,
} from '@/pages/modules/SIM-RS/component/updated-detail/data/resolver.tsx'
...
export const FormUserProfileSIMRS = (props: Props) => {
  ...
  const HandleSave = async (value: IUserProfileResolver) => {
    setLoading(true)
    await AxiosClient.put('/simrs/auth/profile', value)
      .then((res) => {
        if (res?.data?.status) {
          setEdit(!edit)
          setLoading(false)
