...
import type { TRResepObatCPPT } from '../data/resolver.tsx'
...
export const MedicineSelectCPPT = ({ form }: Props) => {
  const [open, setOpen] = useState(false)
  const [search, setSearch] = useState('')
  const daftarResepObat: TRResepObatCPPT[] = form.watch('daftar_resep_obat') ?? []

  const { medicine, loading } = UseGetMedicine({ page: '1', limit: '50', search })

  const handleAddMedicine = (item: IMedicine) => {
    const exists = daftarResepObat.some((r: TRResepObatCPPT) => r.id_obat === item.id_obat)
    if (exists) return
  ...
  const handleRemove = (index: number) => {
    const updated = daftarResepObat.filter((_: TRResepObatCPPT, i: number) => i !== index)
  ...
  const handleFieldChange = (
    ...
    const updated = daftarResepObat.map((item: TRResepObatCPPT, i: number) =>
        ...
        {daftarResepObat.length > 0 && (
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
                  ...
                  <th className="border px-3 py-2 text-center">
                    <div className="flex items-center gap-1">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>
                          Masukkan aturan frekuensi penggunaan obat. Contoh: 3x sehari
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  </th>
                  <th className="border px-3 py-2 text-center">
                    <div className="flex items-center gap-1">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>
                          Masukkan lama penggunaan obat. Contoh: 5 hari
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  </th>
                  <th className="border px-3 py-2 text-center">
                    <div className="flex items-center gap-1">
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent>
                          Masukkan total obat yang diberikan kepada pasien
                        </TooltipContent>
                      </Tooltip>
                    </div>
                  </th>
              ...
              <tbody>
                {daftarResepObat.map((item: TRResepObatCPPT, index: number) => (
                  <tr key={item.id_obat} className="hover:bg-gray-50">
                    ...
                    <td className="border px-3 py-2 text-right font-medium">
                      {formatRupiah((item.harga ?? 0) * item.jumlah)}
                    </td>
                    <td className="border px-3 py-2 text-center">
                      <button
                        type="button"
