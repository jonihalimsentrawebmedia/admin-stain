...
import { ColumnsInpatientReport } from './data/columns.tsx'
...
export const InpatientReportPage = () => {
  ...
  const { stats, loading: loadingStats } = UseGetInpatientReportStats()
  const { list, meta, loading } = UseGetInpatientReportList({ page, limit, search, id_poli, id_dokter, status })

  const { poli } = UseGetPoli({ limit: '100' })
  const { doctor } = UseGetDoctor({ limit: '100' })
  ...
  const columns = ColumnsInpatientReport()

  const cards = [
    {
      title: 'Total Rawat Inap',
    ...
    {
      title: 'Menunggu Ruangan',
      value: stats?.total_menunggu_ruangan ?? 0,
      icon: Clock,
      iconBg: 'bg-yellow-50',
    ...
    },
    {
      ...
      value: stats?.total_sedang_dirawat ?? 0,
      icon: DoorOpen,
      iconBg: 'bg-cyan-50',
      ...
      <TableCustom
        addFilter={
          <div className="flex flex-col sm:flex-row gap-4">
            <SelectFilter name="id_poli" label="Poli" options={poliData} selectClassName="w-full sm:min-w-[200px]" />
            <SelectFilter name="id_dokter" label="Dokter" options={doctorData} selectClassName="w-full sm:min-w-[200px]" />
            <SelectFilter name="status" label="Status" options={statusOptions} selectClassName="w-full sm:min-w-[200px]" />
          </div>
        }
        data={list}
        columns={columns}
