...
import { UseGetPrescription, UseGetPrescriptionStatusCount } from './hooks/index.tsx'
...
export const PrescriptionPage = () => {
  ...
  const activeTab = searchParams.get('status_resep') ?? 'MENUNGGU'
  const page = searchParams.get('page') ?? '1'
  const limit = searchParams.get('limit') ?? '10'
  const search = searchParams.get('search') ?? ''

  const { statusCount } = UseGetPrescriptionStatusCount()
  const { prescription, meta, loading } = UseGetPrescription({
    page,
    limit,
    search,
    status_resep: activeTab,
  })

  const columns = ColumnsPrescription()

  const handleTabChange = (value: string) => {
    setSearchParams((prev) => {
      const newParams = new URLSearchParams(prev)
      newParams.set('status_resep', value)
