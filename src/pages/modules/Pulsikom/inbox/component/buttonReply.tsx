...
import type { IInboxMessage } from '../data/types.ts'
...
export const ButtonReplyInboxMessage = (props: Props) => {
  const { data } = props
  ...
  const HandleReply = async (value: IMessageResolver) => {
    setLoading(true)
    await AxiosClient.post(`/pusilkom/pertanyaan/${data?.id_pertanyaan}/${status}`, value)
      .then((res) => {
        if (res.data.status) {
          setOpen(false)
          setLoading(false)
