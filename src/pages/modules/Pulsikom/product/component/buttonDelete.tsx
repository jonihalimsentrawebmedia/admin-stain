...
export const ButtonDeleteProduct = (props: Props) => {
  const { data } = props
  ...
  const handleDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/pusilkom/produk/${data?.id_produk}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
    ...
    <>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-[12rem_1fr] gap-3 sm:gap-5">
          ...
          <RenderHTMLContent className={'line-clamp-3'} content={data?.deskripsi_lengkap} />
        </div>
