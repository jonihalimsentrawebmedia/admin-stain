...
export const UpdatedAgendaPulsikomPage = () => {
  ...
  const { id } = useParams()
  const { agendaUnitDetail: detail } = UseGetAgendaDetail(id ?? '')
  ...
  const HandleSave = async (e: AgendaType) => {
    setLoading(true)
    await AxiosClient.put(`/pusilkom/agenda/${detail?.id_agenda}`, {
      ...e,
      ...
      .then((res) => {
        if (res.data.status) {
          ...
          setLoading(false)
          navigate('/modules/pulsikom/public-content/agenda')
        }
      })
      .catch((err) => {
