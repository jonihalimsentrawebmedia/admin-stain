...
import { UseGetCategoryDownloadPulsikom, UseGetDownloadPulsikomDetail } from '../hooks/index'
import {
  DownloadResolver,
  type DownloadType,
} from '@/pages/modules/website-utama/public-content/download/types/resolver.tsx'

export const UpdatedDownload = () => {
  const { id } = useParams()
  const { download: detail } = UseGetDownloadPulsikomDetail(id ?? '')
  const [loading, setLoading] = useState(false)

  const { categoryDownload, loading: load1 } = UseGetCategoryDownloadPulsikom({ isGetAll: true })
  ...
  const HandleSubmit = async (e: DownloadType) => {
    setLoading(true)
    await AxiosClient.put(`/pusilkom/downloads/${detail?.id_download}`, e)
      .then((res) => {
        if (res?.data?.status) {
          setLoading(false)
          navigate('/modules/pulsikom/public-content/download')
