...
export const FormContactAndMoreNoted = (props: IProps) => {
  ...
  const { contact } = UseGetContactAndMoreNoteProgram(id)
  ...
  const HandleSave = async (value: TContactMoreNoted) => {
    setLoading(true)
    await AxiosClient.post(`/pusilkom/program/${id}/kontak-dan-catatan-tambahan`, value)
      .then((res) => {
        if (res.data.status) {
          setLoading(false)
          toast.success(res.data.message || 'Success')
          if (from === 'detail') {
            navigate(`/modules/pulsikom/training/credit-earning/program/detail/${id}`)
          } else {
