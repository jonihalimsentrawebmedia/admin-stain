...
import { UseGetNotification } from '../notification/hooks/index'
...
export const NotificationList = () => {
  ...
  const HandleReply = async (value: IMessageResolver) => {
    setLoading(true)
    await AxiosClient.post(`/pusilkom/pertanyaan/${data?.id_pertanyaan}/kirim-jawaban`, value)
      .then((res) => {
        if (res.data.status) {
          setLoading(false)
          setOpen(false)
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-72 sm:w-80 p-2 rounded shadow-lg border bg-white">
