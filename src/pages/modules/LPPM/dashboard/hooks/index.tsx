...
export const UseGetTotalVisitor = () => {
  const { data, isFetching, isLoading } = useQuery<ITotalVisitor>({
    queryKey: ['total-visitor-lppm'],
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get('/lppm/dashboard/total-pengunjung').then((res) => res.data?.data),
  })
...
export const UseGetApprovedList = (status: string) => {
  ...
  const { data, isLoading, isFetching } = useQuery<IContent[]>({
    queryKey: ['list-approved-lppm', ParamsSearch.toString()],
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get(`/lppm/dashboard/list-konten-pengajuan?${ParamsSearch}`).then(
        (res) => res.data.data
      ),
  })
...
export const UseGetTrentVisitor = (mode: Mode) => {
  const { data, isLoading, isFetching } = useQuery<ITrentVisitorResponse>({
    queryKey: ['trent-visitor-lppm', mode],
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get(`/lppm/dashboard/statistik-pengunjung/${mode ?? 'harian'}`).then(
        (res) => res.data?.data
      ),
  })
