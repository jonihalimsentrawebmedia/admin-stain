...
import { UseGetAgendaLppmDetail } from '../hooks'

export const UpdatedAgendaLppmPage = () => {
  ...
  const HandleSave = async (e: AgendaType) => {
    setLoading(true)
    await AxiosClient.put(`/lppm/agenda/${detail?.id_agenda}`, {
      ...e,
      ...
      .then((res) => {
        if (res.data.status) {
          ...
          setLoading(false)
          navigate('/modules/lppm/public-content/agenda')
        }
      })
      .catch((err) => {
