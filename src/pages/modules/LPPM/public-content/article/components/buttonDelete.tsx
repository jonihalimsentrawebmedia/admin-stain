...
export const ButtonDeleteArticleLppm = (data: IArtikel) => {
  ...
  const HandlerDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/lppm/artikel/${data?.id_artikel}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
          toast.success(res.data.message || 'Success menghapus data Artikel')
  ...
  return (
    <>
      ...
      <DialogCustom
        ...
        setOpen={setOpen}
        title={<p className={'text-red-500'}>Hapus Data Artikel?</p>}
        description={'Apakah anda yakin untuk menghapus data Artikel ini?'}
      >
