...
export const UpdatedDataLPPM = () => {
  ...
  const { dataLPPM } = UseGetDetailDataLPPM()

  useEffect(() => {
    if (dataLPPM) {
      const temp: any = { ...dataLPPM }
      form.reset({
        ...temp,
        parent_id_temp: dataLPPM.id_parent_satuan_organisasi,
      })
    }
  }, [dataLPPM])

  const handleSave = async (e: SatuanOrganisasiType) => {
    await AxiosClient.post('/lppm/profil/draft', {
      ...e,
    }).then((res) => {
      if (res.data.status) {
      ...
      <div className={'flex flex-col gap-4'}>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSave)} className="flex flex-col gap-4">
            ...
            />
            <SatuanOrganisasiForm form={form} kelompok="LEMBAGA" />
          </form>
        </Form>
      </div>
    </>
