...
export const ButtonEditDocumentResearchPlan = (data: IPlanResearchDocument) => {
  ...
  const HandleAddDocument = async (e: PlanResearchDocument) => {
    setLoading(true)
    await AxiosClient.put(`/lppm/rencana-induk-penelitian/${data?.id_rencana_induk_penelitian}`, e)
      .then((res) => {
        if (res.data.status) {
          setLoading(false)
          setOpen(false)
