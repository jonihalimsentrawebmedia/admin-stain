...
interface FormProps {
  ...
  handleSubmit: (data: schemaGuideBook) => void
  loading: boolean
  open: boolean
  setOpen: Dispatch<SetStateAction<boolean>>
}

export const FormDocumentGuideBook = (props: FormProps) => {
  const { form, handleSubmit, loading, open, setOpen } = props
  const { guideCategory } = UseGetGuideCategory({
    page: '0',
    limit: '0',
  })
      ...
      <Form {...form}>
        <form className={'flex flex-col gap-5'} onSubmit={form.handleSubmit(handleSubmit)}>
          <SelectBasicInput
            ...
            isRow
            isDisabled
            data={guideCategory?.map((row) => ({
              label: row?.nama_kategori,
              value: row?.id_buku_panduan_kategori,
            }))}
          />
