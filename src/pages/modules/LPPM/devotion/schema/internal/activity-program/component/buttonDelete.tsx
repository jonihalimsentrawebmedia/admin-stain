...
export const ButtonDeleteActivityProgram = (data: IListActivityProgram) => {
  ...
  const HandleDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/lppm/daftar-skema/${data?.id_daftar_skema}`)
      .then((res) => {
        if (res.data.status) {
  ...
  return (
    <>
      ...
      <DialogCustom
        ...
        title={<p className={'text-2xl text-red-500'}>Hapus Program Kegiatan</p>}
      >
        <div className={'grid grid-cols-[12rem_1fr] gap-4'}>
          <p className="text-gray-500">Program Kegiatan*</p>
          <p>{data?.judul}</p>
