...
export const ButtonDeleteDataBRIN = (data: IListBRIN) => {
  ...
  const HandleDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/lppm/brin/${data?.id_brin}`)
      .then((res) => {
        if (res.data.status) {
  ...
  return (
    <>
      ...
      <DialogCustom
        ...
        title={<p className={'text-2xl text-red-500'}>Hapus Pendanan BRIN</p>}
      >
        <div className={'grid grid-cols-[12rem_1fr] gap-4'}>
          <p className="text-gray-500">Pendanan BRIN*</p>
          <p>{data?.judul}</p>
