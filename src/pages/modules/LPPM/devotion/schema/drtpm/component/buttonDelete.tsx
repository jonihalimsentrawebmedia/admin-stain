...
export const ButtonDeleteDataDRTPM = (data: IListDRPM) => {
  ...
  const HandleDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/lppm/drtpm/${data?.id_drtpm}`)
      .then((res) => {
        if (res.data.status) {
  ...
  return (
    <>
      ...
      <DialogCustom
        ...
        title={<p className={'text-2xl text-red-500'}>Hapus Pendanan DRTPM</p>}
      >
        <div className={'grid grid-cols-[12rem_1fr] gap-4'}>
          <p className="text-gray-500">Pendanan DRTPM*</p>
          <p>{data?.judul}</p>
