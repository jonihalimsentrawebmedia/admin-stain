...
export const UpdatedAnnouncementUnit = () => {
  ...
  const HandleSave = async (e: AnnouncementType) => {
    setLoading(true)
    await AxiosClient.put(`/unit/pengumuman/${detail?.id_pengumuman}`, e)
      .then((res) => {
        if (res.data.status) {
          setLoading(false)
          toast.success(res.data.message || 'Pengumuman berhasil dibuat')
          if (from === 'detail') {
            navigate(`/modules/website-unit/public-content/announcement/detail/${id}`)
          } else {
            navigate('/modules/website-unit/public-content/announcement')
          }
        }
      })
      .catch((err) => {
