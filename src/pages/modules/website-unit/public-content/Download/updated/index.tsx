...
import { UseGetCategoryDownloadUnit, UseGetDownloadUnitDetail } from '../hooks/index'
import {
  DownloadResolver,
  type DownloadType,
} from '@/pages/modules/website-utama/public-content/download/types/resolver.tsx'

export const UpdatedDownloadUnitPage = () => {
  const { id } = useParams()
  const { downloadProdiDetail: detail } = UseGetDownloadUnitDetail(id ?? '')
  ...
  const HandleSubmit = async (e: DownloadType) => {
    setLoading(true)
    await AxiosClient.put(`/unit/downloads/${detail?.id_download}`, e)
      .then((res) => {
        if (res?.data?.status) {
          setLoading(false)
