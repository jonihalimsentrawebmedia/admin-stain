...
export const UseGetApprovedListUnit = (status: string) => {
  const ParamsSearch = new URLSearchParams()
  if (status) ParamsSearch.set('status-publish', status)

  const { data, isLoading, isFetching } = useQuery<IContent[]>({
    queryKey: ['list-approved-unit', ParamsSearch.toString()],
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get(`/unit/dashboard/list-konten-pengajuan?${ParamsSearch}`).then(
        (res) => res.data.data
      ),
  })
...
export const UseGetTrentVisitorUnit = (mode: Mode) => {
  const { data, isLoading, isFetching } = useQuery<ITrentVisitor>({
    queryKey: ['trent-visitor-unit', mode],
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get(`/unit/dashboard/statistik-pengunjung/${mode ?? 'harian'}`).then(
        (res) => res.data?.data
      ),
  })
