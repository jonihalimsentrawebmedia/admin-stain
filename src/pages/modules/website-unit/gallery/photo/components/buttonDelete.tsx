...
export const ButtonDeletePhotoUnit = (props: IProps) => {
  ...
  const HandlerDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/unit/galeri-foto/${data?.id_galeri_foto}`)
      .then((res) => {
        if (res.data.status) {
          setOpen(false)
          setLoading(false)
  ...
  return (
    <>
      ...
      <DialogCustom
        ...
        setOpen={setOpen}
        title={
        <p className={'text-2xl text-red-500'}>Hapus Foto</p>
        }
