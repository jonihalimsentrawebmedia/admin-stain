...
import type { LandingList } from '../model/index.tsx'

interface Props {
  data: LandingList|any
  link?: string
  keyList?: string
  isActive?: boolean
}
...
const ButtonSwitch = ({ data, link, keyList, isActive }: Props) => {
  ...
  const linkTemp = link ?? `/lembaga/background/${data.id_lembaga_background}/toggle-status`
      ...
      .then((res) => {
        if (res.data.status) {
          queryClient.invalidateQueries({
            queryKey: [keyList ?? 'landing-page-pengaturan'],
          })
