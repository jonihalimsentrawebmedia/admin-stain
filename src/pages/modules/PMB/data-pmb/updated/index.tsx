...
export const UpdatedDataPMB = () => {
  ...
  const { carrierCenter } = UseGetDetailDataPMB()
  ...
  const handleSave = async (e: SatuanOrganisasiType) => {
    await AxiosClient.post('/pmb/profil/draft', {
      ...e,
    }).then((res) => {
      if (res.data.status) {
        toast.success(res.data.message || 'Success Pengajuan update data universitas')
        navigate('/modules/pmb/data-pmb')
      }
    })
  }
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleSave)} className="flex flex-col gap-4">
            <ButtonTitleGroup
              ...
              label="Update Satuan Pengawas Internal"
            />
