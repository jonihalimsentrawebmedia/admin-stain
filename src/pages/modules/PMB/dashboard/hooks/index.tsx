...
export const UseGetTotalVisitor = () => {
  const { data, isFetching, isLoading } = useQuery<ITotalVisitor>({
    queryKey: ['total-visitor-pmb'],
    refetchOnWindowFocus: false,
    queryFn: () => AxiosClient.get('/pmb/dashboard/total-pengunjung').then((res) => res.data?.data),
  })
...
export const UseGetApprovedList = (status: string) => {
  ...
  const { data, isLoading, isFetching } = useQuery<null | IContent[]>({
    queryKey: ['list-approved-pmb', ParamsSearch.toString()],
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get(`/pmb/dashboard/list-konten-pengajuan?${ParamsSearch}`).then(
        (res) => res.data.data
      ),
  })
...
export const UseGetTrentVisitor = (mode: Mode) => {
  const { data, isLoading, isFetching } = useQuery<ITrentVisitorResponse>({
    queryKey: ['trent-visitor-pmb', mode],
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get(`/pmb/dashboard/statistik-pengunjung/${mode ?? 'harian'}`).then(
        (res) => res.data?.data
      ),
  })
