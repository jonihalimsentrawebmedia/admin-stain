...
import { UseLetterDetailSRB } from './hooks.tsx'
...
const DetailDataSRB = () => {
  const { id } = useParams()
  const { letter, loading } = UseLetterDetailSRB(id as string)

  const [pdfUrl, setPdfUrl] = useState<string | null>(null)
  const [pdfLoading, setPdfLoading] = useState(false)
  const [downloadLoading, setDownloadLoading] = useState(false)
  const [letterData, setLetterData] = useState<ISRBLetter | null>(null)
  ...
  const generatePdf = async (data: ISRBLetter) => {
    setPdfLoading(true)
    try {
      ...
      if (headerData) {
        ...
        try {
          ...
          console.warn('[Detail SRB] Gagal konversi logo ke base64:', e)
        }

        cleanupPdfUrl()

        const pdfDefinition = GenerateSRBLetter({
          data,
          header: headerData,
          logo: logoBase64,
        })
        ...
        console.warn('[Detail SRB] Header kopsurat tidak ditemukan')
        toast.warning('Header kopsurat tidak ditemukan')
      }
    } catch (err) {
      console.error('[Detail SRB] Gagal generate PDF:', err)
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[Download SRB] Gagal konversi logo:', e)
      }
      ...
      console.error('[Download SRB] Gagal download PDF:', err)
  ...
  return (
    <div className="space-y-5">
      <ButtonTitleGroup
        isBack
        label={'Detail Surat Rekomendasi Beasiswa'}
        buttonGroup={[
          {
            type: 'custom',
            element: (
      ...
      <Card className="rounded">
        ...
        <CardContent>
          ...
          <div className="grid grid-cols-[12rem_1fr_12rem_1fr] gap-x-6 gap-y-3">
            ...
            <p>{`${new Date().getFullYear() - Number(letter.angkatan) + 1}`}</p>
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <p>
            Surat Rekomendasi Beasiswa akan ditulis berdasarkan data yang Anda masukkan. Harap
