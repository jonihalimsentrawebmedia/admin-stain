...
import { GenerateSRBLetter } from './pdfgenerate.ts'
...
const UpdateSuratRekomendasiBeasiswaPage = () => {
  ...
  const HandlePreview = async (value: TResolverSRB) => {
    setLoading(true)
    try {
      ...
      } as unknown as ISRBLetter

      const pdfDefinition = GenerateSRBLetter({ logo: logoBase64, data, header: selectedHeader ?? ({} as ILetterHeader) })
