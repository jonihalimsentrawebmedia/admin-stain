...
import { UseLetterDetailSPK } from './hook.tsx'
import { generateSPKLetter } from './pdfgenerate.ts'
...
const DetailSuratPengantarKKN = () => {
  const { id } = useParams()
  const { letter, loading } = UseLetterDetailSPK(id as string)

  const [pdfUrl, setPdfUrl] = useState<string | null>(null)
  const [pdfLoading, setPdfLoading] = useState(false)
  const [downloadLoading, setDownloadLoading] = useState(false)
  const [letterData, setLetterData] = useState<ISPKLetter | null>(null)
  ...
  const generatePdf = async (data: ISPKLetter) => {
    setPdfLoading(true)
    try {
      let headerData = letterHeader

      if (!headerData && data?.kop_surat) {
        headerData = data.kop_surat as unknown as ILetterHeader
      ...
      if (headerData) {
        ...
        try {
          ...
          console.warn('[Detail SPK] Gagal konversi logo ke base64:', e)
        }

        cleanupPdfUrl()

        // @ts-ignore
        const blob = await pdfmake.createPdf(generateSPKLetter(data, logoBase64)).getBlob()
        const url = URL.createObjectURL(blob)
        pdfUrlRef.current = url
        setPdfUrl(url)
      } else {
        console.warn('[Detail SPK] Header kopsurat tidak ditemukan')
        toast.warning('Header kopsurat tidak ditemukan')
      }
    } catch (err) {
      console.error('[Detail SPK] Gagal generate PDF:', err)
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[Download SPK] Gagal konversi logo:', e)
      }

      // @ts-ignore
      const blob = await pdfmake.createPdf(generateSPKLetter(letterData, logoBase64)).getBlob()
      ...
      console.error('[Download SPK] Gagal download PDF:', err)
  ...
  if (loading) {
    return (
      <div className="space-y-5">
        <ButtonTitleGroup isBack label="Detail Surat Pengantar KKN" buttonGroup={[{ type: 'custom', element: <ButtonGoToGuide titleGuide={'Detail Surat'} valueGuide="E_OFFICE_LETTER_LIST" /> }]} />
        <Card>
          <CardHeader>
  ...
  return (
    <div className="space-y-5">
      <ButtonTitleGroup
        isBack
        label="Detail Surat Pengantar KKN"
        buttonGroup={[
          {
            type: 'custom',
            element: (
      ...
      <Card className="rounded">
        ...
        <CardContent>
          <div className="grid grid-cols-[12rem_1fr_12rem_1fr] gap-x-6 gap-y-3">
            ...
            <p>{letter.lampiran === 0 ? '-' : letter.lampiran}</p>
          ...
          <div className="grid grid-cols-[12rem_1fr_12rem_1fr] gap-x-6 gap-y-3">
            ...
            <p>
              {letter.nip_penandatangan || '-'} / {letter.nidn_penandatangan || '-'}
            </p>
          ...
          <div className="space-y-2">
            {letter.mahasiswa_list?.length > 0 ? (
              letter.mahasiswa_list.map((mhs, idx) => (
                <div key={mhs.id_mahasiswa} className="border rounded p-3">
                  <p className="text-xs text-gray-400 mb-1">Mahasiswa #{idx + 1}</p>
                  <div className="grid grid-cols-[12rem_1fr_12rem_1fr] gap-x-6 gap-y-1">
                    <p className="text-gray-500">Nama</p>
                    ...
                    <p>{mhs.nama_fakultas}</p>
                    <p className="text-gray-500">Jenjang</p>
                    <p>{mhs.nama_jenjang}</p>
                  </div>
                </div>
              ))
            ) : (
              <p className="text-gray-400">Tidak ada data mahasiswa</p>
            )}
          </div>

          <hr className="my-4" />
          <p className="text-gray-500 font-semibold mb-2">Data KKN</p>
          <div className="grid grid-cols-[12rem_1fr_12rem_1fr] gap-x-6 gap-y-3">
            <p className="text-gray-500">Tanggal Mulai</p>
            <p>
              {letter.tanggal_mulai
                ? format(new Date(letter.tanggal_mulai), 'dd MMMM yyyy', { locale: localeId })
                : '-'}
            </p>
            ...
            <p>
              {letter.tanggal_selesai
                ? format(new Date(letter.tanggal_selesai), 'dd MMMM yyyy', { locale: localeId })
                : '-'}
            </p>

            <p className="text-gray-500">Desa</p>
            <p>{letter.nama_desa || '-'}</p>

            <p className="text-gray-500">Kecamatan</p>
            <p>{letter.kecamatan || '-'}</p>

            <p className="text-gray-500">Kabupaten</p>
            <p>{letter.kabupaten || '-'}</p>
          </div>

          <hr className="my-4" />
          <p className="text-gray-500 font-semibold mb-2">Dosen Pembimbing Lapangan (DPL)</p>
          <div className="space-y-2">
            {letter.dpl_detail?.length > 0 ? (
              letter.dpl_detail.map((dpl, idx) => (
                <div key={dpl.id_sdm} className="border rounded p-3">
                  <p className="text-xs text-gray-400 mb-1">DPL #{idx + 1}</p>
                  <div className="grid grid-cols-[12rem_1fr_12rem_1fr] gap-x-6 gap-y-1">
                    <p className="text-gray-500">Nama</p>
                    <p>{dpl.nama}</p>
              ...
              <p className="text-gray-400">Tidak ada data DPL</p>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <p>
            Surat Pengantar KKN akan ditulis berdasarkan data yang Anda masukkan. Harap periksa
            kembali untuk menghindari kesalahan penulisan surat.
          </p>
...
export default DetailSuratPengantarKKN
