...
import { GenerateLetterSKPP } from './pdfgenerate.ts'
...
const DetailLetterSPPPage = () => {
  ...
  const [letterData, setLetterData] = useState<ISPPLetter | null>(null)
  ...
  const generatePdf = async (data: ISPPLetter) => {
    setPdfLoading(true)
    try {
      ...
      if (headerData) {
        ...
        try {
          ...
          console.warn('[Detail SPP] Gagal konversi logo ke base64:', e)
        }
        ...
        console.warn('[Detail SPP] Header kopsurat tidak ditemukan')
        toast.warning('Header kopsurat tidak ditemukan')
      }
    } catch (err) {
      console.error('[Detail SPP] Gagal generate PDF:', err)
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[Download SPP] Gagal konversi logo:', e)
      }
      ...
      console.error('[Download SPP] Gagal download PDF:', err)
  ...
  if (!letter) {
    return (
      <div className="space-y-5">
        <ButtonTitleGroup isBack label={'Detail Surat Pengantar Penelitian'} buttonGroup={[{ type: 'custom', element: <ButtonGoToGuide titleGuide={'Detail Surat'} valueGuide="E_OFFICE_LETTER_LIST" /> }]} />
        <Card>
  ...
  return (
    <div className="space-y-5">
      <ButtonTitleGroup
        isBack
        label={'Detail Surat Pengantar Penelitian'}
        buttonGroup={[
          {
            type: 'custom',
            element: (
      ...
      <Card className="rounded">
        ...
        <CardContent>
          ...
          <hr className="my-4" />
          <p className="text-gray-500 font-semibold mb-2">Data Penelitian</p>
          <div className="grid grid-cols-[12rem_1fr_12rem_1fr] gap-x-6 gap-y-3">
            <p className="text-gray-500">Judul Penelitian</p>
            <p>{letter.judul_penelitian || '-'}</p>

            <p className="text-gray-500">Lokasi Penelitian</p>
            <p>{letter.lokasi_penelitian || '-'}</p>

            <p className="text-gray-500">Lama Penelitian</p>
            <p>{letter.lama_penelitian || '-'}</p>

            <p className="text-gray-500">Instansi Tujuan</p>
            <p>{letter.instansi_pimpinan || '-'}</p>

            <p className="text-gray-500">Metode Pengumpulan Data</p>
            <p>{letter.metode_pengumpulan_data?.join(', ') || '-'}</p>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <p>
            Surat Pengantar Penelitian akan ditulis berdasarkan data yang Anda masukkan. Harap
