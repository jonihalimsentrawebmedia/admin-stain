...
import { UseLetterDetailSKAK } from './hook.tsx'
...
const DetailDataSKAK = () => {
  const { id } = useParams()
  const { letter, loading } = UseLetterDetailSKAK(id as string)

  const [pdfUrl, setPdfUrl] = useState<string | null>(null)
  const [pdfLoading, setPdfLoading] = useState(false)
  const [downloadLoading, setDownloadLoading] = useState(false)
  const [letterData, setLetterData] = useState<ISKAKLetter | null>(null)
  ...
  const generatePdf = async (data: ISKAKLetter) => {
    setPdfLoading(true)
    try {
      ...
      if (headerData) {
        ...
        try {
          ...
          console.warn('[Detail SKAK] Gagal konversi logo ke base64:', e)
        }

        cleanupPdfUrl()

        const pdfDefinition = GenerateLetterSKAK({
          data,
          header: headerData,
          logo: logoBase64,
        })
        ...
        console.warn('[Detail SKAK] Header kopsurat tidak ditemukan')
        toast.warning('Header kopsurat tidak ditemukan')
      }
    } catch (err) {
      console.error('[Detail SKAK] Gagal generate PDF:', err)
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[Download SKAK] Gagal konversi logo:', e)
      }
      ...
      console.error('[Download SKAK] Gagal download PDF:', err)
  ...
  if (!letter) {
    return (
      <div className="space-y-5">
        <ButtonTitleGroup
          isBack
          label={'Detail Surat Keterangan Aktif Kembali'}
          buttonGroup={[]}
        />
        <Card>
  ...
  return (
    <div className="space-y-5">
      ...
      <Card className="rounded">
        ...
        <CardContent>
          <div className="grid grid-cols-[12rem_1fr_12rem_1fr] gap-x-6 gap-y-3">
            ...
            <p className="text-gray-500">Semester Cuti</p>
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <p>
            Surat Keterangan Aktif Kembali akan ditulis berdasarkan data yang Anda masukkan. Harap
