...
import { UseLetterDetailSKBP } from './hook.tsx'
import { GenerateSKBPLetter } from '@/pages/modules/E-Office/Letter-Generation/letter-list/detail/SKBP/pdfgenerate.ts'
...
const DetailDataSKBP = () => {
  const { id } = useParams()
  const { letter, loading } = UseLetterDetailSKBP(id as string)

  const [pdfUrl, setPdfUrl] = useState<string | null>(null)
  const [pdfLoading, setPdfLoading] = useState(false)
  const [downloadLoading, setDownloadLoading] = useState(false)
  const [letterData, setLetterData] = useState<ISKBPLetter | null>(null)
  ...
  const generatePdf = async (data: ISKBPLetter) => {
    setPdfLoading(true)
    try {
      ...
      if (headerData) {
        ...
        try {
          ...
          console.warn('[Detail SKBP] Gagal konversi logo ke base64:', e)
        }

        cleanupPdfUrl()

        const pdfDefinition = GenerateSKBPLetter({
          data,
          header: headerData,
          logo: logoBase64,
        })
        ...
        console.warn('[Detail SKBP] Header kopsurat tidak ditemukan')
        toast.warning('Header kopsurat tidak ditemukan')
      }
    } catch (err) {
      console.error('[Detail SKBP] Gagal generate PDF:', err)
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[Download SKBP] Gagal konversi logo:', e)
      }
      ...
      console.error('[Download SKBP] Gagal download PDF:', err)
  ...
  if (!letter) {
    return (
      <div className="space-y-5">
        <ButtonTitleGroup
          isBack
          label={'Detail Surat Keterangan Bebas Pustaka'}
    ...
    <div className="space-y-5">
      ...
      <Card className="rounded">
        ...
        <CardContent>
          ...
          <hr className="my-4" />
          <p className="text-gray-500 font-semibold mb-2">Ketentuan Bebas Pustaka</p>
          <ol className="list-decimal list-inside space-y-1 text-sm">
            {letter.ketentuan_bebas_pustaka?.map((item, idx) => (
              <li key={idx}>{item}</li>
            ))}
          </ol>
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <p>
            Surat Keterangan Bebas Pustaka akan ditulis berdasarkan data yang Anda masukkan. Harap
