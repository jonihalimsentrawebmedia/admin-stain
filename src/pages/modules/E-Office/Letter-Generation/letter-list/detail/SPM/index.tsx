...
import { UseLetterDetailSPM } from './hook.tsx'
...
const DetailSuratPermohonanMagang = () => {
  const { id } = useParams()
  const { letter, loading } = UseLetterDetailSPM(id as string)

  const [pdfUrl, setPdfUrl] = useState<string | null>(null)
  const [pdfLoading, setPdfLoading] = useState(false)
  const [downloadLoading, setDownloadLoading] = useState(false)
  const [letterData, setLetterData] = useState<ISPMLetter | null>(null)
  ...
  const generatePdf = async (data: ISPMLetter) => {
    setPdfLoading(true)
    try {
      ...
      if (headerData) {
        ...
        try {
          ...
          console.warn('[Detail SPM] Gagal konversi logo ke base64:', e)
        }

        cleanupPdfUrl()

        // @ts-ignore
        const blob = await pdfmake.createPdf(generateSPMLetter(data, logoBase64)).getBlob()
        const url = URL.createObjectURL(blob)
        pdfUrlRef.current = url
        setPdfUrl(url)
      } else {
        console.warn('[Detail SPM] Header kopsurat tidak ditemukan')
        toast.warning('Header kopsurat tidak ditemukan')
      }
    } catch (err) {
      console.error('[Detail SPM] Gagal generate PDF:', err)
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[Download SPM] Gagal konversi logo:', e)
      }

      // @ts-ignore
      const blob = await pdfmake.createPdf(generateSPMLetter(letterData, logoBase64)).getBlob()
      ...
      console.error('[Download SPM] Gagal download PDF:', err)
  ...
  return (
    <div className="space-y-5">
      <ButtonTitleGroup
        isBack
        label="Detail Surat Permohonan Magang/PKL"
        buttonGroup={[
          {
            type: 'custom',
            element: (
      ...
      <Card className="rounded">
        ...
        <CardContent>
          <div className="grid grid-cols-[12rem_1fr_12rem_1fr] gap-x-6 gap-y-3">
            ...
            <p className="text-gray-500">Instansi Pimpinan</p>
            <p>{letter.instansi_pimpinan || '-'}</p>

            <p className="text-gray-500">Alamat Instansi</p>
            <p>{letter.alamat_instansi || '-'}</p>

            <p className="text-gray-500">Ditujukan Ke</p>
            <p>{letter.masukan_di || '-'}</p>
              ...
              letter.mahasiswa_list.map((mhs, idx) => (
                <div key={mhs.id_mahasiswa} className="border rounded p-3">
                  ...
                  <div className="grid grid-cols-[12rem_1fr_12rem_1fr] gap-x-6 gap-y-1">
                    ...
                    <p>{mhs.nama_jenjang_pendidikan}</p>
                  </div>
                </div>
              ))
            ) : (
          ...
          <hr className="my-4" />
          <p className="text-gray-500 font-semibold mb-2">Data Kegiatan Magang</p>
          <div className="grid grid-cols-[12rem_1fr_12rem_1fr] gap-x-6 gap-y-3">
            ...
            <p className="text-gray-500">Lama Kegiatan</p>
            <p>{letter.lama_kegiatan ? `${letter.lama_kegiatan} bulan` : '-'}</p>
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <p>
            Surat Permohonan Magang/PKL akan ditulis berdasarkan data yang Anda masukkan. Harap
...
export default DetailSuratPermohonanMagang
