...
import { UseLetterDetailSKBA } from './hook.tsx'
import { GenerateSKBALetter } from '@/pages/modules/E-Office/Letter-Generation/letter-list/detail/SKBA/pdfgenerate.ts'
...
const DetailDataSKBA = () => {
  const { id } = useParams()
  const { letter, loading } = UseLetterDetailSKBA(id as string)

  const [pdfUrl, setPdfUrl] = useState<string | null>(null)
  const [pdfLoading, setPdfLoading] = useState(false)
  const [downloadLoading, setDownloadLoading] = useState(false)
  const [letterData, setLetterData] = useState<ISKBALetter | null>(null)
  ...
  const generatePdf = async (data: ISKBALetter) => {
    setPdfLoading(true)
    try {
      ...
      if (headerData) {
        ...
        try {
          ...
          console.warn('[Detail SKBA] Gagal konversi logo ke base64:', e)
        }

        cleanupPdfUrl()

        const pdfDefinition = GenerateSKBALetter({
          data,
          header: headerData,
          logo: logoBase64,
        })
        ...
        console.warn('[Detail SKBA] Header kopsurat tidak ditemukan')
        toast.warning('Header kopsurat tidak ditemukan')
      }
    } catch (err) {
      console.error('[Detail SKBA] Gagal generate PDF:', err)
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('[Download SKBA] Gagal konversi logo:', e)
      }
      ...
      console.error('[Download SKBA] Gagal download PDF:', err)
    ...
    <div className="space-y-5">
      ...
      <Card className="rounded">
        ...
        <CardContent>
          ...
          <hr className="my-4" />
          <p className="text-gray-500 font-semibold mb-2">Daftar Kewajiban Akademik</p>
          <ol className="list-decimal list-inside space-y-1 text-sm">
            {letter.daftar_kewajiban_akademik?.map((item, idx) => (
              <li key={idx}>{item}</li>
            ))}
          </ol>
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <p>
            Surat Keterangan Bebas Akademik akan ditulis berdasarkan data yang Anda masukkan. Harap
