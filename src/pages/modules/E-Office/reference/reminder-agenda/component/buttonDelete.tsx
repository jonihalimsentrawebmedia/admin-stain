...
const ButtonDeleteReminderAgenda = (props: props) => {
  const { data } = props
  ...
  const HandleSave = async () => {
    setLoading(true)
    await AxiosClient.delete(`/eoffice/waktu-pengingat-agenda/${data.id_waktu_pengigat_agenda}`)
      .then((res) => {
        if (res.data.status) {
          setLoading(false)
          setOpen(false)
...
export default ButtonDeleteReminderAgenda
