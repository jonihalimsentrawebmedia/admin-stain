...
export const ResolverStudentData = z.object({
  ...
  nama_mahasiswa: z.string({ error: 'Nama Mahasiswa harus diisi' }).min(1, 'Nama Mahasiswa harus diisi'),
  id_mahasiswa_unit: z.string({ error: 'Program Studi harus dipilih' }).min(1, 'Program Studi harus dipilih'),
  id_mahasiswa_status: z.string({ error: 'Status Mahasiswa harus dipilih' }).min(1, 'Status Mahasiswa harus dipilih'),
  ...
  id_mahasiswa_jalur_masuk: z.string({ error: 'Jalur Masuk harus dipilih' }).min(1, 'Jalur Masuk harus dipilih'),
  nik: z.string({ error: 'NIK harus diisi' }).length(16, { message: 'NIK harus 16 digit' }),
  jenis_kelamin: z.string({ error: 'Jenis Kelamin harus dipilih' }).min(1, 'Jenis Kelamin harus dipilih'),
  ...
  tanggal_lahir: z.string({ error: 'Tanggal Lahir harus diisi' }).min(1, 'Tanggal Lahir harus diisi'),
  no_hp: z.string({ error: 'No HP harus diisi' }).min(1, 'No HP harus diisi'),
  email: z.string({ error: 'Email harus diisi' }).email('Email tidak valid'),
  ...
  url_foto_mahasiswa: z.string({ error: 'Foto harus diupload' }).min(1, 'Foto harus diupload'),
})
