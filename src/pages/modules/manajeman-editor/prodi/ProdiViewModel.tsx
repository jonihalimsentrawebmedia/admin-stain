  ...
  const columns: ColumnDef<SatuanOrganisasiList>[] = [
    ...
    { accessorKey: 'nama', header: 'Nama Prodi' },
    {
      ...
      cell: (row) => {
        ...
        return (
          <div>
            {values.kode_jenjang}-{values.nama_jenjang_pendidikan}
          </div>
        )
      },
    },
      ...
      cell: (row) => {
        ...
        return (
          <div className="flex gap-2 items-center">
            ...
            <Link to={`/modules/editor/prodi/detail/${values.id_satuan_organisasi}`}>
              <IconDetail />
            </Link>
            {/* Tombol Kuning (Asumsi: Edit) */}
            <Link to={`/modules/editor/prodi/edit/${values.id_satuan_organisasi}`}>
              <IconEdit />
            </Link>
          </div>
        )
