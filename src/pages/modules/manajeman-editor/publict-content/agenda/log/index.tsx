...
import { UseGetAgendaManagementEditorDetail, UseGetLogAgendaManagementEditor } from '../hooks/index'

export const LogActivityAgendaManagementEditorPage = () => {
  const { id } = useParams()
  const columns = ColumnsLog()
  const { logData } = UseGetLogAgendaManagementEditor(id ?? '')
  ...
  return (
    <>
      ...
      <div className={'flex items-start gap-x-8 mt-5'}>
        <img src={detail?.gambar} className={'w-[320px] h-60 object-contain'} alt="imgae" />
