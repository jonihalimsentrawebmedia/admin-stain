...
import {
  UseGetLogAnnouncementManagementEditor,
  UseGetManagementEditorAnnouncementDetail,
} from '@/pages/modules/manajeman-editor/publict-content/announcement/hooks'

export const LogActivityAnnouncementManagementEditorPage = () => {
  const { id } = useParams()
  const columns = ColumnsLog()
  const { logData } = UseGetLogAnnouncementManagementEditor(id ?? '')
    ...
    <>
      ...
      <div className={'flex items-start gap-x-8 mt-5'}>
        <div className={'flex flex-col gap-y-2.5 text-sm'}>
          ...
          <p>{detail?.judul_pengumuman}</p>
          <p className="text-gray-500">isi</p>
          <div className={'p-5 max-h-[250px] overflow-auto border bg-white'}>
            <div
