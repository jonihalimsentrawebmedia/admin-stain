...
export const ButtonDeleteNewsManagementEditor = (data: INewsDetail) => {
  ...
  const HandlerDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/editor/berita/${data?.id_berita}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
          toast.success(res.data.message || 'Success menghapus data berita')
  ...
  return (
    <>
      ...
      <DialogCustom
        ...
        setOpen={setOpen}
        title={'Hapus Berita?'}
        description={'Apakah anda yakin untuk menghapus berita yang ditulis?'}
      >
