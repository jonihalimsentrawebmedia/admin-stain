...
import { UseGetCarrierNewsDetail } from '../hooks/index.tsx'
import { formatDate } from 'date-fns'

export const NewsCarrierUpdated = () => {
  const { id } = useParams()
  const { unitNewsDetail } = UseGetCarrierNewsDetail(id ?? '')
  ...
  useEffect(() => {
    if (unitNewsDetail) {
      ...
      form.reset({
        ...
        tanggal_berita: formatDate(unitNewsDetail?.tanggal_berita, 'yyyy-MM-dd'),
  ...
  const HandleSubmit = async (e: INewsTypeForm) => {
    setLoading(true)
    await AxiosClient.put(`/pusat-karir/berita/${id}`, {
      ...e,
      ...
      .then((res) => {
        if (res.data.status) {
          ...
          setLoading(false)
          navigate('/modules/pusat-karir/public-content/news')
        }
      })
      .catch((err) => {
  ...
  return (
    <>
      <FormNewsContent
        url_ai={'/pusat-karir/berita/generate'}
        loading={loading}
        form={form}
