...
import { UseGetCategoryDownloadCarrier, UseGetDownloadCarrierDetail } from '../hooks/index'
import {
  DownloadResolver,
  type DownloadType,
} from '@/pages/modules/website-utama/public-content/download/types/resolver.tsx'

export const UpdatedDownloadCarrierPage = () => {
  const { id } = useParams()
  const { downloadProdiDetail: detail } = UseGetDownloadCarrierDetail(id ?? '')
  const [loading, setLoading] = useState(false)

  const { categoryDownload, loading: load1 } = UseGetCategoryDownloadCarrier({ isGetAll: true })
  ...
  const HandleSubmit = async (e: DownloadType) => {
    setLoading(true)
    await AxiosClient.put(`/pusat-karir/downloads/${detail?.id_download}`, e)
      .then((res) => {
        if (res?.data?.status) {
          setLoading(false)
          navigate('/modules/pusat-karir/public-content/download')
