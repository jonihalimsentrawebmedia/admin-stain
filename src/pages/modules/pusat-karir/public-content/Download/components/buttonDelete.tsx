...
export const ButtonDeleteFileDownloadCarrier = (props: props) => {
  const { data } = props
  ...
  const HandlerDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/pusat-karir/downloads/${data?.id_download}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
          toast.success(res.data.message || 'Success menghapus data Download')
          queryClient.invalidateQueries({ queryKey: ['download-carrier'] })
        }
      })
      .catch((err) => {
        setLoading(false)
