...
export const ButtonDeleteCategoryDownloadCarrier = (data?: ICategoryDownload) => {
  ...
  const HandleSubmit = async () => {
    setLoading(true)
    await AxiosClient.delete(`/pusat-karir/kategori-berkas/${data?.id_kategori_berkas}`)
      .then((res) => {
        if (res.data.status) {
          setOpen(false)
          setLoading(false)
          toast.success(res.data.message)
          queryClient.invalidateQueries({ queryKey: ['category-download-carrier'] })
        }
      })
      .catch((err) => {
        setLoading(false)
