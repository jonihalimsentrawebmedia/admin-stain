import { Link, useSearchParams } from 'react-router-dom'
import type { ColumnDef } from '@tanstack/react-table'
import { Button } from '@/components/ui/button.tsx'
import { MdInfo, MdOutlineHistory } from 'react-icons/md'
import { Carousel, CarouselContent, CarouselItem } from '@/components/ui/carousel.tsx'
import { format } from 'date-fns'
import { TimeAgo } from '@/utils/helper.tsx'
import { HiPencil } from 'react-icons/hi'
import { ButtonPublishArticle } from '../buttonPublish'
import Autoplay from 'embla-carousel-autoplay'
import type { IArticleCarrier } from '../../data/types.ts'
import { ButtonDeleteArticle } from '../buttonDelete.tsx'

export const UnPublishColumnsUnit = () => {
  const [searchParams] = useSearchParams()
  const page = Number(searchParams.get('page') ?? 1)
  const limit = Number(searchParams.get('limit') ?? 10)

  const columns: ColumnDef<IArticleCarrier>[] = [
    {
      accessorKey: 'No',
      header: '#',
      cell: ({ row }) => {
        const i = row?.index
        return <>{(page - 1) * limit + i + 1}</>
      },
    },
    {
      accessorKey: 'gambar',
      header: 'Gambar',
      cell: ({ row }) => {
        return (
          <div>
            {row?.original?.artikel_gambar_tambahan.length > 0 ? (
              <Carousel
                className={'w-[100px]'}
                plugins={[
                  Autoplay({
                    delay: 2000,
                  }),
                ]}
              >
                <CarouselContent className={'w-fit'}>
                  {row?.original?.artikel_gambar_tambahan.map((item, index) => (
                    <CarouselItem key={index}>
                      <img
                        src={item?.gambar}
                        className={'w-[100px] h-[75px] object-cover'}
                        alt={item?.keterangan}
                      />
                    </CarouselItem>
                  ))}
                </CarouselContent>
              </Carousel>
            ) : (
              <img
                src={row.original?.gambar}
                alt={row.original?.judul}
                className={'w-[100px] h-[75px] object-cover'}
              />
            )}
          </div>
        )
      },
    },
    {
      accessorKey: 'judul',
      header: 'Judul',
      cell: ({ row }) => {
        return <p className={'text-sm whitespace-pre-line'}>{row?.original?.judul}</p>
      },
    },
    {
      accessorKey: 'nama_kategori_berita',
      header: 'Kategori Berita',
    },
    {
      accessorKey: 'penulis',
      header: 'Penulis',
    },
    {
      accessorKey: 'unpublished_at',
      header: 'Tgl. UnPublish',
      cell: ({ row }) => {
        return (
          <>
            <div className="flex flex-col gap-1.5 text-center">
              <p className={'text-sm'}>
                {format(row?.original?.unpublished_at as string, 'dd MMMM yyyy')}
              </p>
              <p className={'text-sm'}>
                {format(row?.original?.unpublished_at as string, 'HH:mm:ss')}
              </p>
              <p className={'text-primary text-sm'}>
                {TimeAgo(row?.original?.unpublished_at as string)}
              </p>
            </div>
          </>
        )
      },
    },
    {
      accessorKey: 'action',
      header: 'Aksi',
      cell: ({ row }) => {
        return (
          <div className={'flex flex-col gap-1.5 items-center'}>
            <Link to={`edit/${row?.original?.id_artikel}`}>
              <Button
                size={'sm'}
                variant={'outline'}
                className={'text-yellow-500 border-yellow-500 hover:text-yellow-500'}
              >
                <HiPencil />
                Perbaiki
              </Button>
            </Link>
            <ButtonPublishArticle {...row?.original} />
          </div>
        )
      },
    },
    {
      accessorKey: 'log',
      header: 'Log',
      cell: ({ row }) => {
        return (
          <>
            <Link to={`log/${row?.original?.id_artikel}`}>
              <Button
                size={'sm'}
                variant={'outline'}
                className={'text-blue-500 border-blue-500 hover:text-blue-500'}
              >
                <MdOutlineHistory />
                Lihat Log
              </Button>
            </Link>
          </>
        )
      },
    },
    {
      accessorKey: 'action',
      header: 'Aksi',
      cell: ({ row }) => {
        return (
          <div className={'flex flex-col gap-1.5 items-center'}>
            <ButtonDeleteArticle {...row?.original} />
            <Link to={`detail/${row?.original?.id_artikel}`}>
              <button className={'bg-blue-500 p-1.5 rounded text-white hover:bg-blue-600'}>
                <MdInfo />
              </button>
            </Link>
          </div>
        )
      },
    },
  ]

  return columns
}
