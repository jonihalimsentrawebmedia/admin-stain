...
export const ButtonSubmissionArticle = (data?: IArticleCarrier) => {
    ...
    await AxiosClient.patch(`/pusat-karir/artikel/${data?.id_artikel}/status-publish`, {
      ...
      .then((res) => {
        if (res?.data?.status) {
          ...
          queryClient.invalidateQueries({
            queryKey: ['carrier-article-detail'],
          })
        }
      })
      .catch((err) => {
    ...
    <>
      ...
      >
        <div className={'flex flex-col gap-2.5'}>
          {data?.artikel_gambar_tambahan && data?.artikel_gambar_tambahan.length > 0 ? (
            <Carousel>
              <CarouselContent>
