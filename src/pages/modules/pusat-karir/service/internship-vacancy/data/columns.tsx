...
import type { IInternshipVacancy } from './types'
import { format } from 'date-fns'
import { HiPencil } from 'react-icons/hi'
import { MdInfo } from 'react-icons/md'
import { FaUserFriends } from 'react-icons/fa'
import { Button } from '@/components/ui/button.tsx'
import { ButtonDeleteInternshipVacancy } from '../component/buttonDelete'

export const ColumnsInternship = () => {
  const [searchParams] = useSearchParams()
  const page = Number(searchParams.get('page') ?? '1')
  const limit = Number(searchParams.get('limit') ?? '10')

  const columns: ColumnDef<IInternshipVacancy>[] = [
    {
      accessorKey: 'no',
      header: 'No',
      cell: ({ row }) => {
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1.5">
          ...
          <ButtonDeleteInternshipVacancy data={row?.original} />
        </div>
      ),
    },
  ]
