  ...
  return (
    <>
      <div className="border w-full p-2.5 rounded bg-white space-y-2">
        ...
        <Input
          className={'rounded'}
          value={'judul' in bagianActive ? (bagianActive?.judul ?? '') : ''}
          onChange={(e) => {
      ...
      {('pertanyaan' in bagianActive ? (bagianActive?.pertanyaan ?? []) : [])?.map(
        (row: IPertanyaanSurvey | undefined, k: number) => (
          <div
            key={k}
        ...
        <ButtonTitleGroup
          ...
          buttonGroup={[
            ...
            },
            {
              type: 'custom',
              element: (
                <Button className={'rounded'} onClick={actionSave}>
                  <FaSave />
                  Simpan Survei
                </Button>
              ),
