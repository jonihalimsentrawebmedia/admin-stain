...
export const UseGetTotalVisitor = () => {
  const { data, isFetching, isLoading } = useQuery<ITotalVisitor>({
    queryKey: ['total-visitor-carrier'],
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get('/pusat-karir/dashboard/total-pengunjung').then((res) => res.data?.data),
  })
...
export const UseGetApprovedList = (status: string) => {
  ...
  const { data, isLoading, isFetching } = useQuery<null | IContent[]>({
    queryKey: ['list-approved-carrier', ParamsSearch.toString()],
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get(`/pusat-karir/dashboard/list-konten-pengajuan?${ParamsSearch}`).then(
        (res) => res.data.data
      ),
  })
...
interface ITrentVisitorResponse {
  tren_kunjungan: any
...
export const UseGetTrentVisitor = (mode: Mode) => {
  const { data, isLoading, isFetching } = useQuery<ITrentVisitorResponse>({
    queryKey: ['trent-visitor-carrier', mode],
    refetchOnWindowFocus: false,
    queryFn: () =>
      AxiosClient.get(`/pusat-karir/dashboard/statistik-pengunjung/${mode ?? 'harian'}`).then(
        (res) => res.data?.data
      ),
  })
