...
export const UpdatedNewsSPI = () => {
  const { id } = useParams()
  const { unitNewsDetail } = UseGetNewsDetail(id ?? '')
  ...
  const HandleSubmit = async (e: INewsTypeForm) => {
    setLoading(true)
    await AxiosClient.put(`/spi/berita/${id}`, {
      ...e,
      ...
      .then((res) => {
        if (res.data.status) {
          ...
          setLoading(false)
          navigate('/modules/spi/public-content/news')
        }
      })
      .catch((err) => {
