...
import { UseGetDocumentSystemDetail } from '@/pages/modules/SPI/quality-assurance/document-system/hooks'

export const EditDocumentSystem = () => {
  ...
  useEffect(() => {
    if (detail) {
      form.reset({
        ...
        id_kategori_sistem_dokumen: detail?.id_kategori_sistem_dokumen,
  ...
  const HandleSubmit = async (e: DownloadType) => {
    setLoading(true)
    await AxiosClient.put(`/spi/sistem-dokumen/${id}`, e)
      .then((res) => {
        if (res?.data?.status) {
          setLoading(false)
