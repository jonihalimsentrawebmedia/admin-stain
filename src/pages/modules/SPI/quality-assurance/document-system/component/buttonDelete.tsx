...
import type { ISystemDocument } from '../data/types'
import { useForm } from 'react-hook-form'
import { Form } from '@/components/ui/form.tsx'
import TextInput from '@/components/common/form/TextInput.tsx'

interface props {
  data: ISystemDocument
}

export const ButtonDeleteSystemDocument = (props: props) => {
  const { data } = props
  ...
  const HandlerDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/spi/sistem-dokumen/${data?.id_sistem_dokumen}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
          toast.success(res.data.message || 'Success menghapus data Download')
          queryClient.invalidateQueries({ queryKey: ['document-system'] })
        }
      })
      .catch((err) => {
        setLoading(false)
  ...
  return (
    <>
      ...
      <DialogCustom
        open={open}
        setOpen={setOpen}
        title={<p className={'text-red-500 text-2xl'}>Hapus Sistem Dokumen</p>}
        description={'Apakah anda yakin untuk menghapus Sistem Dokumen ini?'}
      ...
      >
        <div className={'grid grid-cols-[12rem_1fr] gap-4'}>
          ...
          <div className="flex items-center justify-end col-span-2 gap-2">
            ...
            <Button
              disabled={loading || form.watch('validator') !== 'DELETE'}
