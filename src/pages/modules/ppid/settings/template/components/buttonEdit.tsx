...
import type { ThemaPPID } from '../model/index'
...
const ButtonEditDescription = (props: Props) => {
  ...
  const HandleSave = async (e: any) => {
    setLoading(true)
    await AxiosClient.post(`/unit-ppid/thema/${data?.thema}/keterangan`, e)
      .then((res) => {
        if (res.data.status) {
          setLoading(false)
          setOpen(false)
          toast.success(res.data.message || 'Success Pengajuan update data keterangan thema')
          queryClient.invalidateQueries({ queryKey: ['template-pengaturan-unit-ppid'] })
        }
      })
      .catch((err) => {
        setLoading(false)
