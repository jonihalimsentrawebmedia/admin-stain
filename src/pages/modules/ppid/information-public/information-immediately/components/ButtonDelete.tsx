...
const ButtonDelete = ({ data }: Props) => {
  ...
  async function handleDelete() {
    ...
    try {
      const res = await AxiosClient.delete(
        `/unit-ppid/informasi-serta-merta-informasi/${data.id_informasi_serta_merta}`
      )
      ...
      if (res.data.status) {
        ...
        await queryClient.invalidateQueries({
          queryKey: ['information-immediately-unit-ppid'],
        })
      }
    } catch (err: any) {
  ...
  return (
    <>
      ...
      >
        <img
          className="min-w-[360px] max-w-[360px] text-center mx-auto max-h-[360px] min-h-[360px] object-cover w-full h-full"
          src={data.url_gambar}
          alt={data.key_gambar}
        />
        <div className="my-4 ">
