...
interface Props {
  data: any
  linkDelete: string
  keyLinkDelete: string
  title: string
  titleField?: string
}
const ButtonDeleteDocument = ({ data, keyLinkDelete, linkDelete, title, titleField }: Props) => {
  ...
  const fieldsConfigUrl: any = [
    {
      name: 'judul',
      label: 'Judul*',
      isHidden: !titleField ? true : false,
    },
    {
      name: 'nama_dokumen',
      label: 'Nama Dokumen*',
  ...
  async function handleDelete() {
    setLoading(true)
    try {
      const res = await AxiosClient.delete(linkDelete)

      if (res.data.status) {
        toast.success(res.data.message)

        await queryClient.invalidateQueries({
          queryKey: [keyLinkDelete],
        })
      }
    } catch (err: any) {
  ...
  return (
    <>
      ...
      <DialogCustom
        ...
        open={open}
        setOpen={setOpen}
        title={<p className="text-2xl text-red-500">Hapus {title}</p>}
      >
