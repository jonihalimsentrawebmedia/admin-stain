...
const StandartServiceViewModel = () => {
  const [searchParams] = useSearchParams()
  const page = Number(searchParams.get('page') || 1)
  const limit = Number(searchParams.get('limit') || 10)
  const columns: ColumnDef<PublicInformationServiceStandardList>[] = [
    // ✅ Nomor (#)
    {
      accessorKey: 'no',
      header: '#',
        ...
        return (
          <div className="flex gap-2 items-center">
            <ButtonEditDocument
              ...
              linkPost={`/unit-ppid/standard-pelayanan-informasi-public/${values.id_standard_pelayanan_informasi_public}`}
              data={values}
            />
            <ButtonDeleteDocument
              titleField={undefined}
              title="Dokumen"
              keyLinkDelete="ppip-standart-service"
              linkDelete={`/unit-ppid/standard-pelayanan-informasi-public/${values.id_standard_pelayanan_informasi_public}`}
              data={values}
            />
          </div>
        )
...
export default StandartServiceViewModel
