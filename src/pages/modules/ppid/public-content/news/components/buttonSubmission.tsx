...
export const ButtonSubmissionNews = (data: INewsDetail) => {
  ...
  const HandleSubmission = async () => {
    setLoading(true)
    await AxiosClient.patch(`/unit-ppid/berita/${data?.id_berita}/status-publish`, {
      ...
      .then((res) => {
        if (res?.data?.status) {
          ...
          queryClient.invalidateQueries({
            queryKey: ['list-news-unit-ppid'],
          })
          queryClient.invalidateQueries({
            queryKey: ['detail-news-unit-ppid'],
          })
          queryClient.invalidateQueries({
