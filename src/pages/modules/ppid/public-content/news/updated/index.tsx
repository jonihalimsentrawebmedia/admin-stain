...
export const UpdatedNewsPage = () => {
  ...
  const HandleSubmit = async (e: INewsTypeForm) => {
    ...
    await AxiosClient.put(`/unit-ppid/berita/${detailNews?.id_berita}`, {
      ...e,
      tanggal_berita: new Date(e?.tanggal_berita).toISOString(),
    })
      .then((res) => {
        if (res.data.status) {
          navigate('/modules/ppid/public-content/news')
          setLoading(false)
          toast.success(res.data.message || 'Success Pengajuan update data berita')
        }
      })
      .catch((err) => {
  ...
  return (
    <>
      <FormNewsContent
        url_ai={'/unit-ppid/berita/generate'}
        loading={loading}
        form={form}
