...
import { UseGetFacultyNewsDetail } from '../hooks/index.tsx'
import { formatDate } from 'date-fns'

export const NewsFacultyUpdated = () => {
  const { id } = useParams()
  const { unitNewsDetail } = UseGetFacultyNewsDetail(id ?? '')
  ...
  const HandleSubmit = async (e: INewsTypeForm) => {
    setLoading(true)
    await AxiosClient.put(`/fakultas/berita/${id}`, {
      ...e,
  ...
  return (
    <>
      <FormNewsContent
        url_ai={'/fakultas/berita/generate'}
        loading={loading}
        form={form}
