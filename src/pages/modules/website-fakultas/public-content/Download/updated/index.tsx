...
import { UseGetCategoryDownloadFaculty, UseGetDownloadFacultyDetail } from '../hooks/index'
import {
  DownloadResolver,
  type DownloadType,
} from '@/pages/modules/website-utama/public-content/download/types/resolver.tsx'

export const UpdatedDownloadFacultyPage = () => {
  const { id } = useParams()
  const { downloadProdiDetail: detail } = UseGetDownloadFacultyDetail(id ?? '')
  const [loading, setLoading] = useState(false)

  const { categoryDownload, loading: load1 } = UseGetCategoryDownloadFaculty({ isGetAll: true })
  ...
  useEffect(() => {
    if (detail) {
      form.reset({
        nama_berkas: detail?.nama_berkas,
        id_kategori_berkas: detail?.id_kategori_berkas,
        is_link_drive: detail?.is_link_drive,
        link_drive: detail?.link_drive,
  ...
  const HandleSubmit = async (e: DownloadType) => {
    setLoading(true)
    await AxiosClient.put(`/fakultas/downloads/${detail?.id_download}`, e)
      .then((res) => {
        if (res?.data?.status) {
          setLoading(false)
          navigate('/modules/website-fakultas/public-content/download')
