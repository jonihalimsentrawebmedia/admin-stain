...
export const ButtonEditCategoryDownloadFaculty = (data?: ICategoryDownload) => {
  ...
  const HandleSubmit = async (e: any) => {
    setLoading(true)
    await AxiosClient.put(`/fakultas/kategori-berkas/${data?.id_kategori_berkas}`, e)
      .then((res) => {
        if (res.data.status) {
          setOpen(false)
          setLoading(false)
          toast.success(res.data.message)
          queryClient.invalidateQueries({ queryKey: ['category-download-faculty'] })
          form.reset()
        }
      })
      .catch((err) => {
