...
export const ButtonDeletePartnerMitra = (props: Props) => {
  const { data } = props
  ...
  const handleDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/fakultas/mitra-kerjasama/${data?.id_mitra_kerjasama}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
  ...
  return (
    <>
      ...
      <DialogBasic
        ...
        setOpen={setOpen}
      >
        <img src={data?.url_gambar} alt="gambar" className="object-contain w-[150px]" />
