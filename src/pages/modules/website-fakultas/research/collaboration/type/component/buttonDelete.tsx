...
export const ButtonDeleteTypeCollaboration = (props: Props) => {
  const { data } = props
  ...
  const handleDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`/fakultas/bidang-kolaborasi/${data?.id_fakultas_bidang_kolaborasi}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
    ...
    <>
      ...
      >
        <div className="grid grid-cols-[12rem_1fr] gap-5">
          <p className="text-gray-500">Nama Bidang</p>
          <p>{data?.nama}</p>
          <p className="text-gray-500">Deskripsi</p>
          <RenderHTMLContent content={data?.deskripsi} />
        </div>
