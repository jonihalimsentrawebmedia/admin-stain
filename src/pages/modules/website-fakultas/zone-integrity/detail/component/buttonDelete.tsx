...
import type { ISbuZoneIntegrity } from '../data/types.ts'

interface Props {
  data: ISbuZoneIntegrity
}

export const ButtonDeleteSubZoneIntegrity = (props: Props) => {
  const { data } = props
  ...
  const handleDelete = async () => {
    ...
    await AxiosClient.delete(
      `/fakultas/zona-integritas-sub-kategori/${data?.id_zona_integritas_sub_kategori}`
    )
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
    ...
    <>
      ...
      >
        <div className="grid grid-cols-[12rem_1fr]">
          ...
          <p className="text-gray-500">Nama Sub Kategori</p>
          <p>{data?.nama_sub_kategori}</p>
        </div>
