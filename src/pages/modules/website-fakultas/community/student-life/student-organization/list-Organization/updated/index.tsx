...
import { UseGetDetailStudentOrganization } from '../hooks/index'
import ButtonTitleGroup from '@/components/common/button/ButtonTitleGroup.tsx'

export const UpdatedStudentOrganization = () => {
  ...
  const handleSave = async (e: IStudentOrganization) => {
    ...
    await AxiosClient.put(
      `/fakultas/daftar-organisasi-mahasiswa/${detail?.id_daftar_organisasi_mahasiswa}`,
      e
    )
      .then((res) => {
        if (res.data.status) {
    ...
    <>
      <Form {...form}>
        <form className={'space-y-5'} onSubmit={form.handleSubmit(handleSave)}>
          <ButtonTitleGroup
            isBack
            label={'Ubah Data Organisasi Mahasiswa'}
            buttonGroup={[
              {
                type: 'cancel',
                label: 'Batal',
          ...
          />
          <UploadPhotoImage name={'url_gambar'} form={form} label={'Gambar Organisasi Mahasiswa'} />
          <TextInput
            name={'nama'}
            form={form}
