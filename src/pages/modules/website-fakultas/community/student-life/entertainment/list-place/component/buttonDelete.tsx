...
import type { IStudentEntertainment } from '../data/types'
...
export const ButtonDeleteStudentEntertainment = (props: Props) => {
  const { data } = props
  ...
  const handleDelete = async () => {
    ...
    await AxiosClient.delete(
      `/fakultas/daftar-hiburan-mahasiswa/${data?.id_daftar_hiburan_mahasiswa}`
    )
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
    ...
    <>
      ...
      >
        <div className="grid grid-cols-[12rem_1fr]">
          <p className="text-gray-500">Nama Hiburan Mahasiswa</p>
          <p>{data?.nama}</p>
          <p className="text-gray-500">Tentang</p>
          <RenderHTMLContent className={'line-clamp-3'} content={data?.deskripsi} />
        </div>
