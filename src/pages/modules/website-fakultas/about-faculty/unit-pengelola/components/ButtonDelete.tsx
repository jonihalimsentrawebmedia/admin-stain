...
import type { IManagementUnit } from '../data/resolver.tsx'

interface Props {
  data: IManagementUnit
}

const ButtonDeleteUserFaculty = ({ data }: Props) => {
  const form = useForm()
  ...
  const handleDelete = async () => {
    setLoading(true)
    await AxiosClient.delete(`fakultas/profil/unit-pengelola/${data?.id_unit_pengelola}`)
      .then((res) => {
        if (res?.data?.status) {
          setOpen(false)
          setLoading(false)
...
export default ButtonDeleteUserFaculty
